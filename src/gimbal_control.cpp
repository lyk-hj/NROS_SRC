...
//namespace robot_detection{
    ...
    {
        cv::FileStorage fs("/home/lmx/real/other/control_data.yaml", cv::FileStorage::READ);
...
//    std::cout<<"imu_pos: "<<imu_pos<<std::endl;
    ...
    {
        Vector3d tmp_pos;
        tmp_pos = RotationMatrix_imu.transpose() * imu_pos;

        Vector3d cam_pos;
        cam_pos = RotationMatrix_cam2imu.transpose() * tmp_pos;
        return cam_pos;
    }
...
//         std::cout<<"tmp_pixel: "<<tmp_pixel<<std::endl;
        cv::Point2f pixel_pos = Point2f((float)tmp_pixel(0,0)/tmp_pixel(2,0),(float)tmp_pixel(1,0)/tmp_pixel(2,0));
